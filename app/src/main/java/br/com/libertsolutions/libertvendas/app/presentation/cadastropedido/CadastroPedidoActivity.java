...
public class CadastroPedidoActivity extends LibertVendasActivity
    ...
    private void addStepListaProdutos(List<ItemPedido> pItensPedido) {
        if (!mTabAdapter.hasPosition(PAGE_LISTA_PRODUTOS)) {
            mTabAdapter.addFragment(ListaProdutosFragment.newInstance(true, pItensPedido),
                    getString(title_fragment_selecione_produtos));
        }
    }

    private void addStepListaClientes(Cliente pClientePedido) {
