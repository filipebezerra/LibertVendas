...
class OrderListAdapter extends RecyclerView.Adapter<OrderListViewHolder>
    ...
    @Override public void onBindViewHolder(final OrderListViewHolder holder, final int position) {
        ...
        final boolean containsOrderId = orderId != null && orderId != 0;
        holder.textViewOrderNumber.setVisibility(containsOrderId ? VISIBLE : GONE);
        if (containsOrderId) {
            holder.textViewOrderNumber.setText(context.getString(
                    order_list_template_text_order_number, orderId));
        }
